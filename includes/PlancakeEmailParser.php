        ...
        foreach ($this->rawBodyLines as $line) {
            if (!$detectedContentType) {
                ...
                if (is_array($boundaries)) {
                    if (strlen($line) > 2 && in_array(substr($line, 2), $boundaries)) {  // found the delimiter
                        break;
                    }
                    if (strlen($line) > 4 && in_array(substr($line, 2, strlen($line)-4), $boundaries)) {  // found the delimiter
                        break;
                    }
                }
                $body .= $line . "\n";
